...
static void framework_open_output (struct prte_mca_base_framework_t *framework)
{
    if (0 < framework->framework_verbose) {
        ...
        prte_output_set_verbosity(framework->framework_output,
                                       ...
                                       prte_mca_base_register_flag_t flags)
{
    ...
    if (!(PRTE_MCA_BASE_FRAMEWORK_FLAG_NOREGISTER & framework->framework_flags)) {
        ...
        prte_asprintf (&desc, "Default selection set of components for the %s framework (<none>"
        ...
        ret = prte_mca_base_var_register (framework->framework_project, framework->framework_name,
                                           ...
                                           PRTE_MCA_BASE_VAR_SCOPE_ALL_EQ, &framework->framework_selection);
        free (desc);
        if (0 > ret) {
            return ret;
        }
...
int prte_mca_base_framework_register_list (prte_mca_base_framework_t **frameworks, prte_mca_base_register_flag_t flags)
{
    if (NULL == frameworks) {
...
int prte_mca_base_framework_open (struct prte_mca_base_framework_t *framework,
    ...
    ret = prte_mca_base_var_group_find (framework->framework_project,
                                         framework->framework_name,
                                         NULL);
...
int prte_mca_base_framework_open_list (prte_mca_base_framework_t **frameworks, prte_mca_base_open_flag_t flags)
{
    if (NULL == frameworks) {
        return PRTE_ERR_BAD_PARAM;
    }
...
int prte_mca_base_framework_close (struct prte_mca_base_framework_t *framework) {
    ...
    group_id = prte_mca_base_var_group_find (framework->framework_project,
    ...
    if (is_open) {
        ...
        while (NULL != (item = prte_list_remove_first (&framework->framework_components))) {
            ...
            prte_mca_base_component_unload(cli->cli_component,
    ...
    framework->framework_flags &= ~(PRTE_MCA_BASE_FRAMEWORK_FLAG_REGISTERED | PRTE_MCA_BASE_FRAMEWORK_FLAG_OPEN);
